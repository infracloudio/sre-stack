...
RDS_VPC_ID=$(aws eks describe-cluster --name prod-eks-cluster --region ${AWS_REGION} --query "cluster.resourcesVpcConfig.vpcId" --output text)
...
CLUSTER_SG=$(aws eks describe-cluster --name prod-eks-cluster --region ${AWS_REGION} --query "cluster.resourcesVpcConfig.clusterSecurityGroupId" --output text)
