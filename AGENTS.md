...
- `.specify/` — Spec Kit engine (templates, scripts, constitution in
  `.specify/memory/constitution.md`); **generated by the Specify CLI**
  (`specify init` / `specify integration install`) — refresh with the CLI,
  do not hand-edit templates in place
- `specs/<nnn>-<slug>/` — one folder per story: `spec.md`, `plan.md`,
  `tasks.md`; created by `/speckit.specify`, merged with the code
...
- `.claude/skills/speckit-*/`, `.agents/skills/`, `.devin/skills/`,
  `.opencode/commands/` — the Spec Kit workflow for each harness
  (claude, codex, devin, opencode); **generated by the Specify CLI**,
  not by loom — do not hand-edit
