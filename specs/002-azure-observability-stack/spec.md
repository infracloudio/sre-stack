...
- Q: How should the two web screens be reachable from the developer's machine — one public address each, or one shared entry point? → A: One shared entry point: a single reachable address with each screen under its own path. Which ingress mechanism provides that address is a platform-wide decision outside this story — it is owned by the shared-ingress story (#104) and settled in this story's plan, not here. This story installs and maintains no gateway of its own; it only publishes its two screens behind whatever shared entry point the platform provides.
...
- **FR-005**: The two web screens MUST be reachable from the developer's machine through one shared public address, each screen under its own path on that address. The ingress mechanism that provides the address is decided in the plan and provided by the shared-ingress story (#104); this story MUST NOT install or configure a gateway of its own.
...
- The shared entry point (one public address, path routing to each screen) is platform-wide infrastructure provided by the shared-ingress story (#104), not by this story. This story only assumes such an entry point exists or is created by the same start command, attaches its two screens to it under their own paths, and reads the address back for the endpoints command. The ingress mechanism, its release, and where its workloads run are decided in that story and in this story's plan. The single public address is enough — no hostname or certificate in this story. Cleanup is always the whole cluster, so anything the entry point creates is removed with it.
