...
- The cluster's managed ingress feature is the platform's managed service mesh, which brings its own gateway (front door) and uses the newer path-routing standard (the Gateway API), not the older web-server form whose upstream project has been retired. The mesh is chosen over the lighter gateway-only option because the Amazon and local setups already run a service mesh, later Azure stories (applications, traces) will need one, and the platform does not allow both options on one cluster. The platform runs the gateway and applies its own security patches automatically; moving to a newer mesh release is a deliberate step by the operator. Checked on 2026-09-15 against the sandbox subscription: the feature's commands exist in the standard sign-in tool, the fallback location offers a mesh release compatible with the pinned cluster version, no subscription policy blocks a public address, and address/load-balancer quota is unused. "Safe to repeat" means the setup reads whether the feature is already on before asking for it, rather than relying on the platform to ignore a second request. The single public address is enough — no hostname or certificate in this story. The gateway's own workloads run where the platform places them; only the monitoring stack must sit on the observability pool. Cleanup is always the whole cluster, so anything the feature creates is removed with it.
