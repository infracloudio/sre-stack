...
- [x] T050 CRITICAL: Make the allowance check in `infra/scripts/cluster/azure-common.sh` account for the exact cluster and pools already present before requiring capacity for missing resources; preserve the existing workload-pool mode when resuming, and let `infra/scripts/cluster/verify-cluster-aks.sh` verify that mode without requiring free capacity for a second cluster. For example, a regular cluster using all 24 DSv5 and 4 FSv2 allowed vCPU must still pass a no-op rerun and verification; a partly built spot cluster must not gain regular pools because its earlier pools consumed the spot allowance. Add realistic nonzero-current-usage cases to `agent/tests/azure/` and sync the command contract and recorded design per Constitution I, FR-003/FR-011/FR-014, US1/AC2, and T010 (contradicts)
- [x] T051 CRITICAL: Make Azure cleanup in `infra/scripts/cluster/cleanup-cluster.sh` distinguish a confirmed absent resource from a failed read, report failed reads with a nonzero exit, and check the one exact predicted node resource group even when the main group is already absent. For example, a network error must not print "Nothing to clean", and retrying after the main group was deleted must still warn if its node group remains. Keep the existing no-blind-delete rule, add offline read-error and orphan-retry cases in `agent/tests/azure/`, and sync the contract per Constitution I and FR-004/FR-011 (contradicts)
- [x] T052 CRITICAL: Replace the command names and implementation-specific comparison instructions in `specs/001-azure-aks-setup/spec.md` US3 with the plain-language accepted outcome: existing Amazon/local files remain unchanged and the repository checks pass. Keep the accepted substitute for a live Amazon run and its AD-004 reference; retain executable instructions in the plan/quickstart and obtain the required independent acknowledgement of the spec amendment per Constitution VI and T048 (contradicts)
- [x] T053 CRITICAL: Route top-level `make setup` and `make cleanup` in `makefile` through only the Azure empty-cluster lifecycle when `STACK_MODE=aks`, preserving the existing Amazon/local paths. Currently setup still schedules application, observability, and Amazon helpers, while cleanup schedules gateway and RDS removal before Azure cleanup. Add offline coverage of these public make targets, including refusal dispatch, without contacting a cloud, and sync the quickstart/contract per US1/AC1, FR-004/FR-008, and plan: same start command creates an empty cluster (contradicts)
- [x] T054 Make the machine-allowance projection in `infra/scripts/cluster/azure-common.sh` deterministic: use an ordered multiselect list for TSV or parse JSON by field name instead of depending on the order of `{name, what, cur, lim}`. The current reader rejects valid rows ordered `cur, lim, name, what`; Microsoft documents that object-shaped TSV output has no ordering guarantee. Update `agent/tests/azure/fake-az.sh` and the offline checks to enforce the requested shape instead of always returning the reader's preferred order, and sync `specs/001-azure-aks-setup/contracts/azure-cli-contract.md` per FR-014 and T023/T040 (partial)
- [x] T055 Extend the read-only empty-cluster check in `infra/scripts/cluster/verify-cluster-aks.sh` to inspect workloads as well as namespace names and confirm the Kubernetes reads target the intended Azure cluster. For example, an application Deployment in `default` currently passes because only `kubectl get namespaces` is read. Add offline cases for an unexpected workload in an allowed namespace and an incorrect cluster context, preserving the legitimate housekeeping resources, and sync the contract per FR-008/FR-012 and T010 (partial)
- [x] T056 Correct partial-failure reporting in `infra/scripts/cluster/setup-cluster-aks.sh` and `infra/scripts/cluster/cleanup-cluster.sh` to distinguish confirmed resources, operations already submitted, and state that could not be read. For example, after `az aks create --no-wait` succeeds and polling fails, the cluster must not be asserted "Not created"; a failed delete must identify what remains or explicitly say which exact resources could not be checked. Use read-only checks where possible, keep the no-rollback rule, and add offline asynchronous-create and partial-delete failure cases with matching contract updates per FR-011 and T007/T014 (partial)


## Phase 13: Convergence

- [x] T057 CRITICAL: Make existence checks in `infra/scripts/cluster/azure-common.sh` and `infra/scripts/cluster/setup-cluster-aks.sh` distinguish confirmed absence from a failed read before creating a group, cluster, pool, or storage setting. For example, a failed `az group exists` currently reaches `az group create`, and failed cluster/pool reads are also treated as missing resources. Stop with a plain error on unreadable state, preserve existing resources, and add offline read-error cases in `agent/tests/azure/` with matching command-contract updates per Constitution I, FR-003/FR-011, and T007/T050 (contradicts)
- [x] T058 CRITICAL: Correct the machine-allowance JSON parser in `infra/scripts/cluster/azure-common.sh` and the corresponding answer in `agent/tests/azure/fake-az.sh` to match the installed Azure CLI's `az vm list-usage --output json` array of usage items. The helper currently calls `doc.get("value")`, which raises `AttributeError` on that array; the fake hides the failure by returning a REST-style object wrapper. Preserve parsing by field name, handle the CLI's numeric strings, reject malformed answers plainly, and cover fresh setup, resume, and verification with the real output shape; sync the command contract per FR-014, US1/AC1, and T023/T054 (contradicts)
- [x] T059 Complete partial-failure reporting in `infra/scripts/cluster/setup-cluster-aks.sh` and `infra/scripts/cluster/cleanup-cluster.sh`: report resources as absent only after confirming absence, and distinguish attempted operations from confirmed results. For example, after an accepted asynchronous cluster create with unreadable state, its system pool is also unconfirmed, not "Not created"; a failed pool add may have created the pool; a failed group delete does not prove "nothing else was removed", and a failed node-group read after successful deletion must not say "Nothing was deleted". Add offline cases for these uncertain outcomes and sync the command contract per FR-011 and T056 (partial)
- [x] T060 Validate the complete node-pool set in `infra/scripts/cluster/verify-cluster-aks.sh`, rejecting unexpected pools and invalid workload-pool modes as well as missing or malformed expected pools. For example, the current checker reports success for the five expected pools plus a sixth User pool with ten nodes. Add offline extra-pool and wrong-mode cases in `agent/tests/azure/`, preserve the documented system-pool and spot exceptions, and sync the command contract per FR-012, SC-002/SC-004, T010, and plan: one system pool plus four workload pools (partial)
