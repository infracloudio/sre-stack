...
- R10  Istio control-plane components (istiod; istio-base installs CRDs only, no pods) and the ingress-gateway are scheduled on the AKS system node pool via an explicit `nodeSelector` (`kubernetes.azure.com/mode=system`) set on both Helm releases in `setup-istio-aks.sh`. The system pool carries no taint (verified: `sed -n '96p' specs/001-azure-aks-setup/data-model.md` → `| system | Standard_D2s_v5 | 1–1 | — | — | no | System |`), but the app pool is untainted too, so the default scheduler will place untolerated pods on either one — confirmed on a real AKS 1.34 cluster, where both pods landed on the app pool until the `nodeSelector` was added. No tolerations are required (neither pool is tainted); AD-003 (spot-priority toleration) does not apply to this story, since the system pool is not a spot-priced workload pool.
