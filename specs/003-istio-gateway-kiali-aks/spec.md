...
- R4  `make get-service-endpoints` returns the AKS load-balancer external IP in the same environment variable format as EKS: `LB_ENDPOINT` is a bare IP or hostname (e.g., `LB_ENDPOINT=52.xxx.xxx.xxx`, no scheme, no port — corrected this round after an independent audit caught the earlier `http://` claim as wrong)
