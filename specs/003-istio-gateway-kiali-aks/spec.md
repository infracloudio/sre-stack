...
3. **Scenario: Get the gateway address** — A user runs `make get-service-endpoints`: it reads the `istio-ingressgateway` Service (created by `setup-istio`) and returns one reachable load-balancer address for the AKS cluster (e.g., `http://52.xxx.xxx.xxx:80`), in the same format EKS already produces. This is the address later stories (Grafana, Kiali, app routing) will route through.
...
- R2  Correction, verified directly against the real makefile this round: on EKS, `setup-gateway` (line 152) does not create the ingress-gateway Deployment or Service at all — that happens inside `setup-istio`'s third Helm install (line 78). `setup-gateway` on EKS only creates the `robot-shop` namespace and applies `app/robot-shop/Istio/gateway.yaml` (an app-specific Gateway/VirtualService). Since Robot Shop is out of scope for this story (#102), `setup-gateway` on AKS has no app-specific resource to apply yet. R2 is revised: `make setup-gateway` on AKS exists as a target (so the composite `make setup` sequence doesn't break) but is a no-op for this story — it creates no resources, since the load-balancer itself comes from `setup-istio` and no app route exists yet. A later story (#102) gives it real work once Robot Shop lands on AKS.

  This is a deeper correction than the R2/R3 VirtualService wording fixed earlier this round — that fix was still built on a wrong premise (that `setup-gateway` creates the LoadBalancer). This corrects the premise itself, found while resolving the T004/T005 ownership question below.
