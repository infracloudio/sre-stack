...
- [x] **T032** `.env`'s `AZURE_SUBSCRIPTION_ID` held a real subscription ID (`674579f0-...`), not a placeholder — committed on this branch. Decision: reverted to empty (matches its state before this branch touched it). Note for reviewers: this value will still appear in this branch's commit history until squash-merge; a squash-merge to `main` will not carry it forward since the net diff reverts it to empty.
