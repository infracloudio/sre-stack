...
| **Resource requests/limits (datastores)** | `mysql`: requests `100m / 700Mi`, limits `200m / 1024Mi`. `rabbitmq`: requests `500m / 2Gi`, limits `1 CPU / 2Gi`. `mysqlseeder`: requests `100m / 500Mi`, limits `200m / 1Gi`. `mongodb` and `redis` have **no explicit `resources:` block** in `values.yaml` — they run with no requests/limits set (cluster/namespace defaults apply, if any). | Read directly from `values.yaml`. The missing `mongodb`/`redis` limits are a real gap against FR-006's "smallest practical CPU/memory requests" intent — worth a task to add them, or an explicit note that this is accepted as-is for this story. |
